...
main = do
    ...
    nameOverrides
        <- Map.fromList <$> input auto "./config/name_overrides.dhall"
    csvData <- BL.readFile (file params)
    case decodeByName csvData of
        Right ( _, players ) -> do
            players' <- getPlayersWithProjected
                (Config nameOverrides)
                (week params)
                players
            case players' of
                Right ps -> BL.writeFile (out params) $
                    encodeDefaultOrderedByName (toList ps)
                Left err -> print err
        left -> print left
  where
    opts =
