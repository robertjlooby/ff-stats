...
pickBestLineups players = do
    ...
    newTeams
        <- foldM (\ts _ -> nextGeneration ts) teams [ 1 .. iterationRounds ]
    getTop newTeams
  where
...
nextGeneration teams = do
    ...
    logInfo $
        ...
        ++ show maxFitness
        ++ " avg fitness: "
        ++ show (sum fitnesses / fromIntegral (length teams))
    newTeams
...
mutatePair first second = do
    ( first', second' ) <- mutate qb ( first, second )
        >>= mutate rb1
        >>= mutate rb2
        >>= mutate wr1
        >>= mutate wr2
