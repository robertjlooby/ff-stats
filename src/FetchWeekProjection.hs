...
import           Text.XML.Cursor
       ( ($//), (>=>), attributeIs, element, fromDocument, node )

import           Network.Wreq
       ( Response, defaults, getWith, param, responseBody )
...
getProjectedScore name week usePPR = do
    ...
    case outlook of
        Just cursor -> case cursor
            $// (element "span" >=> attributeIs "class" "pull-right") of
                [] -> return $
                    Left $
                    "Player projected score not found for: "
                    <> show name
                    <> " week "
                    <> show week
                cursors -> do
                    ...
                    return score
        Nothing -> return . Left $
            "Player outlook div not found for: "
            <> show name
            <> " week "
            <> show week
