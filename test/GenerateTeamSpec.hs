...
getTeam :: PlayerPool -> IO Team
getTeam pool = runApp (generateTeam pool) undefined =<< newStdGen

spec :: Spec
spec = describe "Teams" $ do
    describe "generate player pool" $ do
        it "separates out the QBs" $ property $ \players -> conjoin $
            (\player -> getPosition player === QB)
            <$> _qbs (generatePlayerPool players)
        it "separates out the RBs" $ property $ \players -> conjoin $
            (\player -> getPosition player === RB)
            <$> _rbs (generatePlayerPool players)
        it "separates out the WRs" $ property $ \players -> conjoin $
            (\player -> getPosition player === WR)
            <$> _wrs (generatePlayerPool players)
        it "separates out the TEs" $ property $ \players -> conjoin $
            (\player -> getPosition player === TE)
            <$> _tes (generatePlayerPool players)
        it "separates out the DSTs" $ property $ \players -> conjoin $
